...
def update_likes_count(sender, instance, **kwargs):
    ...
    if target_object and hasattr(target_object, 'likes_count'):
...
def update_followers_count(sender, instance, **kwargs):
    target_object = instance.content_object
    if target_object and hasattr(target_object, 'followers_count'):
...
def update_comment_count(sender, instance, **kwargs):
    target_object = instance.content_object
    if target_object and hasattr(target_object, 'comment_count'):
        ...
        target_object.comment_count = count
        target_object.save(update_fields=['comment_count'])